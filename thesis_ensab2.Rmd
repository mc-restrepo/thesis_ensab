...
Explorative data analysis steps:
...
examen13 %>%
  filter(MA_208 == 2) %>%
  dplyr::select(ID_PERSONA)

examen13 %>%
  filter(if_any(everything(), is.na)) %>%
  dplyr::select(ID_PERSONA)

#two children have NAs in clinical examination. Variable MA_208 is appropriate to find NAs
...
examen5 %>%
  filter(if_any(everything(), is.na)) %>%
  dplyr::select(ID_PERSONA) #no NAs but we will still exclude participant, everything is 0
```
...
```{r}
#show rows with missin data in clinical portion
...
ensab <- ensab %>%
  ...
  mutate(cariessev3 = factor(cariessev3, levels = c(0, 1, 2), labels = c("No ECC", "Non-severe ECC", "Severe ECC"), ordered = TRUE))
...
6382/6446*100
61/6446*100 #0.9463233 missing data on nutrition

#Total percentage of participants with missing data
(61+3)/6446*100
...
ggplot(ensab, aes(x = age, fill = cariessev3)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(x = "Age (years) ", y = "Prevalence", 
       title = "Figure 1. Prevalence of Early Childhood Caries (ECC) Experience and Severity by Age",
       subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
       caption = "Data source: ENSAB IV") +
  theme_minimal() +
  scale_fill_manual(
    name = "ECC Experience and Severity",
    values = c("Severe ECC" = "#CC3333", "Non-severe ECC" = "#F8766D", "No ECC" = "grey")) +
  theme(legend.position = "right")
```


#Table 3: Weekly consumption of food groups by age
...
ggplot(ensab, aes(x = age, fill = carbs)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(
    x = "Age",
    y = "Proportion",
    title = "Food Groups by Age"
  ) +
  theme_minimal() +
  scale_fill_brewer(palette = "PuRd", name = "Carbohydrate Frequency") +
  theme(legend.position = "top")

fig_dietage <- t2 %>%
  dplyr::select(-Total) %>%
  pivot_longer(cols = -c(Food, Freq), names_to = "Age", values_to = "Percent") %>%
  mutate(
    Proportion = as.numeric(str_extract(Percent, "(?<=\\().*?(?=%)")),
    Freq = factor(Freq, levels = c("Never", "1x", "2-3x", "4-7x", "8+x"), ordered = TRUE)
  )

ggplot(fig_dietage, aes(x = Age, y = Proportion, fill = Freq)) +
  geom_col(position = "fill") +
  facet_wrap(~ Food, ncol = 3) +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_fill_brewer(palette = "Blues", name = "Weekly Consumption") +
  labs(
    x = "Age (years)",
    y = "Proportion (%)",
    title = "Weekly Intake of Food Groups by Age",
    subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
    caption = "Data source: ENSAB IV"
  ) +
  theme_minimal() +
...
# Table 3b. Weekly consumption of food groups by region

Function

```{r}
f_foodgroupsdf <- function(foodgroup, strat_variable) {
  foodgroup_name <- rlang::as_string(foodgroup)
  strat_variable_name <- rlang::as_string(strat_variable)
  
  totaldf <- ensab %>%
    group_by(.data[[foodgroup_name]]) %>%
    summarise(Total = n(), .groups = "drop") %>%
    mutate(Percent = Total / sum(Total)) %>%
    mutate(Total = paste0(round(Total, 3), " (", round(Percent * 100, 2), "%)")) %>%
    select(Freq = .data[[foodgroup_name]], Total)

  stratdf <- ensab %>%
    group_by(.data[[strat_variable_name]], .data[[foodgroup_name]]) %>%
    summarise(Total = n(), .groups = "drop_last") %>%
    mutate(Percent = Total / sum(Total)) %>%
    mutate(Percent_complete = paste0(round(Total, 3), " (", round(Percent * 100, 2), "%)")) %>%
    select(.data[[strat_variable_name]], Freq = .data[[foodgroup_name]], Percent_complete) %>%
    pivot_wider(names_from = .data[[strat_variable_name]], values_from = Percent_complete)

  t_foodgroupsdf <- left_join(totaldf, stratdf, by = "Freq") %>%
    mutate(Food = foodgroup_name) %>%
    select(Food, everything())

  return(t_foodgroupsdf)
}

#lapply with all food groups and region

foodgroups <- rlang::syms(c("carbs", "veg", "fruit", "prot", "dairy", "fats", "sugar", "soda", "juice"))

fgregion <- lapply(foodgroups, function(fg) {
  f_foodgroupsdf(foodgroup = fg, strat_variable = rlang::sym("region"))
})

t_fgregion <- bind_rows(fgregion) #combine
t_fgregion

#export as csv
write.csv(t_fgregion, "t3b_foodgroups_region.csv", row.names = FALSE)

```

## Fig 3. Food groups by region

```{r}
#make the data tidy

fig_dietregion <- t_fgregion %>%
  dplyr::select(-Total) %>%
  pivot_longer(cols = -c(Food, Freq), names_to = "Region", values_to = "Percent") %>%
  mutate(
    Proportion = as.numeric(str_extract(Percent, "(?<=\\().*?(?=%)")),
    Freq = factor(Freq, levels = c("Never", "1x", "2-3x", "4-7x", "8+x"), ordered = TRUE)
  )
...
ggplot(fig_dietregion, aes(x = Region, y = Proportion, fill = Freq)) +
  geom_col(position = "fill") +
  facet_wrap(~ Food, ncol = 3) +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_fill_brewer(palette = "Blues", name = "Weekly Consumption") +
  labs(
    x = "Region",
    y = "Proportion (%)",
    title = "Weekly Intake of Food Groups by Region",
    subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
    caption = "Data source: ENSAB IV"
  ) +
  theme_minimal() +
...
Carbs only with region ordered by highest proportion in category 8+x

```{r}

# Step 1: Create ordered Region factor based on "8+x"
region_order <- fig_dietregion %>%
  filter(Food == "carbs", Freq == "8+x") %>%
  arrange(desc(Proportion)) %>%
  mutate(Region = fct_inorder(Region)) %>%
  pull(Region)
region_order

# Step 2: Apply that ordering to the full dataset
fig_dietregion %>%
  filter(Food == "carbs") %>%
  mutate(Region = factor(Region, levels = region_order)) %>%
  ggplot(aes(x = Region, y = Proportion, fill = Freq)) +
  geom_col(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_fill_brewer(palette = "Blues", name = "Weekly Consumption") +
  labs(
    x = "Region",
    y = "Proportion (%)",
    title = "Weekly Intake of Carbohydrates by Region",
    subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
    caption = "Data source: ENSAB IV"
  ) +
  theme_minimal() +
...
fig_dietregion %>%
  ...
  labs(
    x = "Region",
    y = "Proportion (%)",
    title = "Weekly Intake of Vegetables by Region",
    subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
    caption = "Data source: ENSAB IV"
  ) +
  theme_minimal() +
...
fig_dietregion %>%
  ...
  labs(
    x = "Region",
    y = "Proportion (%)",
    title = "Weekly Intake of Fruits by Region",
    subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
    caption = "Data source: ENSAB IV"
  ) +
  theme_minimal() +
...
fig_dietregion %>%
  ...
  labs(
    x = "Region",
    y = "Proportion (%)",
    title = "Weekly Intake of Protein by Region",
    subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
    caption = "Data source: ENSAB IV"
  ) +
  theme_minimal() +
...
fig_dietregion %>%
  ...
  labs(
    x = "Region",
    y = "Proportion (%)",
    title = "Weekly Intake of Dairy by Region",
    subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
    caption = "Data source: ENSAB IV"
  ) +
  theme_minimal() +
...
fig_dietregion %>%
  ...
  labs(
    x = "Region",
    y = "Proportion (%)",
    title = "Weekly Intake of Fats by Region",
    subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
    caption = "Data source: ENSAB IV"
  ) +
  theme_minimal() +
...
fig_dietregion %>%
  ...
  labs(
    x = "Region",
    y = "Proportion (%)",
    title = "Weekly Intake of Sugars by Region",
    subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
    caption = "Data source: ENSAB IV"
  ) +
  theme_minimal() +
...
fig_dietregion %>%
  ...
  labs(
    x = "Region",
    y = "Proportion (%)",
    title = "Weekly Intake of Soda by Region",
    subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
    caption = "Data source: ENSAB IV"
  ) +
  theme_minimal() +
...
fig_dietregion %>%
  ...
  labs(
    x = "Region",
    y = "Proportion (%)",
    title = "Weekly Intake of Juice by Region",
    subtitle = "Children 1, 3 and 5 Years Old, Colombia 2014",
    caption = "Data source: ENSAB IV"
  ) +
  theme_minimal() +
...
Created in the web version of dagitty: https://www.dagitty.net/dags.html

Copy and paste the code below into the web version to see the DAG with colors and to edit it/move it.
...
dag <- dagitty('dag {
"Age of first dental visit" [pos="0.729,0.362"]
"Area of residence (urban/rural)" [pos="-0.270,1.100"]
"Diet: Weekly consumption of food groups" [exposure,pos="-1.091,0.196"]
"Early childhood caries" [outcome,pos="0.283,0.196"]
"Health insurance coverage" [pos="0.406,0.897"]
"Household income" [pos="-0.366,-1.356"]
"Oral hygiene" [pos="0.719,-0.470"]
"Socioeconomic position" [pos="0.280,-1.058"]
Age [pos="-1.359,-0.828"]
Region [pos="-0.988,0.868"]
Sex [pos="-0.927,-1.243"]
"Age of first dental visit" -> "Early childhood caries"
"Area of residence (urban/rural)" -> "Diet: Weekly consumption of food groups"
"Area of residence (urban/rural)" -> "Early childhood caries"
"Diet: Weekly consumption of food groups" -> "Early childhood caries"
"Health insurance coverage" -> "Age of first dental visit"
"Health insurance coverage" -> "Diet: Weekly consumption of food groups"
"Health insurance coverage" -> "Early childhood caries"
"Household income" -> "Diet: Weekly consumption of food groups"
"Household income" -> "Early childhood caries"
"Oral hygiene" -> "Early childhood caries"
"Socioeconomic position" -> "Diet: Weekly consumption of food groups"
"Socioeconomic position" -> "Early childhood caries"
Age -> "Diet: Weekly consumption of food groups"
Age -> "Early childhood caries"
Region -> "Diet: Weekly consumption of food groups"
Region -> "Early childhood caries"
Sex -> "Diet: Weekly consumption of food groups"
Sex -> "Early childhood caries"
}')
...
Setup: checking cariessev, which should be binary

```{r}
summary(ensab$cariessev)
glimpse(ensab$cariessev) #however, it is 1 and 2

ensab <- ensab %>%
  mutate(cariessev_num = ifelse(cariessev == "Severe ECC", 1, 0))
summary(ensab$cariessev_num)
...
```{r}
m3carbs <- glm(cariessev_num ~ carbs, data = ensab, family = "binomial"(link = "log"))
summary(m3carbs)

m4carbs <- glm(cariessev_num ~ carbs + age + sex + region + ses + income + insurance + location,
             family = poisson(link = "log"), data = ensab)
summary(m4carbs)
coeftest(m4carbs, vcov = sandwich)# Robust standard errors
exp(m4carbs$coefficients) #PR

robust_se <- coeftest(m4carbs, vcov = sandwich)[, 2]  # Extract robust standard errors

pvalue <- coeftest(m4carbs, vcov = sandwich)[, 4]

carbs_ci2 <- data.frame(
  Estimate = round(exp(coef(m4carbs)), 3),
  Lower_CI = round(exp(coef(m4carbs) - 1.96 * robust_se), 3),
  Upper_CI = round(exp(coef(m4carbs) + 1.96 * robust_se), 3),
  P_value = round(pvalue, 4)
) %>%
  mutate(
    PR = paste0(Estimate, " (", Lower_CI, ", ", Upper_CI, ")"),
...
vif(m4carbs) #checking multicollinearity
```

### Veg

```{r}
m3veg <- glm(cariessev_num ~ veg, data = ensab, family = "binomial"(link = "log"))
summary(m3veg)

m4veg <- glm(cariessev_num ~ veg + age + sex + region + ses + income + insurance + location,
             family = poisson(link = "log"), data = ensab)
summary(m4veg)
coeftest(m4veg, vcov = sandwich)# Robust standard errors
exp(m4veg$coefficients) #PR

robust_se <- coeftest(m4veg, vcov = sandwich)[, 2]  # Extract robust standard errors
pvalue <- coeftest(m4veg, vcov = sandwich)[, 4]

veg_ci2 <- data.frame(
  Estimate = round(exp(coef(m4veg)), 3),
  Lower_CI = round(exp(coef(m4veg) - 1.96 * robust_se), 3),
  Upper_CI = round(exp(coef(m4veg) + 1.96 * robust_se), 3),
  P_value = round(pvalue, 4)
) %>%
  mutate(
    PR = paste0(Estimate, " (", Lower_CI, ", ", Upper_CI, ")"),
...
vif(m4veg) #checking multicollinearity
...
```{r}
m3fruit <- glm(cariessev_num ~ fruit, data = ensab, family = "binomial"(link = "log"))
summary(m3fruit)

m4fruit <- glm(cariessev_num ~ fruit + age + sex + region + ses + income + insurance + location,
             family = poisson(link = "log"), data = ensab)
summary(m4fruit)
coeftest(m4fruit, vcov = sandwich)# Robust standard errors
exp(m4fruit$coefficients) #PR

robust_se <- coeftest(m4fruit, vcov = sandwich)[, 2]  # Extract robust standard errors
pvalue <- coeftest(m4fruit, vcov = sandwich)[, 4]

fruit_ci2 <- data.frame(
  Estimate = round(exp(coef(m4fruit)), 3),
  Lower_CI = round(exp(coef(m4fruit) - 1.96 * robust_se), 3),
  Upper_CI = round(exp(coef(m4fruit) + 1.96 * robust_se), 3),
  P_value = round(pvalue, 4)
) %>%
  mutate(
    PR = paste0(Estimate, " (", Lower_CI, ", ", Upper_CI, ")"),
  ...
  dplyr::select(Food, Freq, PR, P_value)
fruit_ci2
vif(m4fruit) #checking multicollinearity
...
```{r}
m3prot <- glm(cariessev_num ~ prot, data = ensab, family = "binomial"(link = "log"))
summary(m3prot)

m4prot <- glm(cariessev_num ~ prot + age + sex + region + ses + income + insurance + location,
             family = poisson(link = "log"), data = ensab)
summary(m4prot)
coeftest(m4prot, vcov = sandwich)# Robust standard errors
exp(m4prot$coefficients) #PR
robust_se <- coeftest(m4prot, vcov = sandwich)[, 2]  # Extract robust standard errors
pvalue <- coeftest(m4prot, vcov = sandwich)[, 4]

prot_ci2 <- data.frame(
  Estimate = round(exp(coef(m4prot)), 3),
  Lower_CI = round(exp(coef(m4prot) - 1.96 * robust_se), 3),
  Upper_CI = round(exp(coef(m4prot) + 1.96 * robust_se), 3),
  P_value = round(pvalue, 4)
) %>%
  mutate(
    PR = paste0(Estimate, " (", Lower_CI, ", ", Upper_CI, ")"),
...
vif(m4prot) #checking multicollinearity
```

### Dairy

```{r}
m3dairy <- glm(cariessev_num ~ dairy, data = ensab, family = "binomial"(link = "log"))
summary(m3dairy)

m4dairy <- glm(cariessev_num ~ dairy + age + sex + region + ses + income + insurance + location,
             family = poisson(link = "log"), data = ensab)
summary(m4dairy)

coeftest(m4dairy, vcov = sandwich)# Robust standard errors
exp(m4dairy$coefficients) #PR
robust_se <- coeftest(m4dairy, vcov = sandwich)[, 2]  # Extract robust standard errors
pvalue <- coeftest(m4dairy, vcov = sandwich)[, 4]

dairy_ci2 <- data.frame(
  Estimate = round(exp(coef(m4dairy)), 3),
  Lower_CI = round(exp(coef(m4dairy) - 1.96 * robust_se), 3),
  Upper_CI = round(exp(coef(m4dairy) + 1.96 * robust_se), 3),
  P_value = round(pvalue, 4)
) %>%
  mutate(
    PR = paste0(Estimate, " (", Lower_CI, ", ", Upper_CI, ")"),
...
vif(m4dairy) #checking multicollinearity
...
```{r}
m3fats <- glm(cariessev_num ~ fats, data = ensab, family = "binomial"(link = "log"))
summary(m3fats)

m4fats <- glm(cariessev_num ~ fats + age + sex + region + ses + income + insurance + location,
             family = poisson(link = "log"), data = ensab)
summary(m4fats)

coeftest(m4fats, vcov = sandwich)# Robust standard errors
exp(m4fats$coefficients) #PR
robust_se <- coeftest(m4fats, vcov = sandwich)[, 2]  # Extract robust standard errors
pvalue <- coeftest(m4fats, vcov = sandwich)[, 4]

fats_ci2 <- data.frame(
  Estimate = round(exp(coef(m4fats)), 3),
  Lower_CI = round(exp(coef(m4fats) - 1.96 * robust_se), 3),
  Upper_CI = round(exp(coef(m4fats) + 1.96 * robust_se), 3),
  P_value = round(pvalue, 4)
) %>%
  mutate(
    PR = paste0(Estimate, " (", Lower_CI, ", ", Upper_CI, ")"),
...
vif(m4fats) #checking multicollinearity
```

### Sugars

```{r}
m3sugar <- glm(cariessev_num ~ sugar, data = ensab, family = "binomial"(link = "log"))
summary(m3sugar)

m4sugar <- glm(cariessev_num ~ sugar + age + sex + region + ses + income + insurance + location,
             family = poisson(link = "log"), data = ensab)
summary(m4sugar)
coeftest(m4sugar, vcov = sandwich)# Robust standard errors
exp(m4sugar$coefficients) #PR
robust_se <- coeftest(m4sugar, vcov = sandwich)[, 2]  # Extract robust standard errors
pvalue <- coeftest(m4sugar, vcov = sandwich)[, 4]

sugar_ci2 <- data.frame(
  Estimate = round(exp(coef(m4sugar)), 3),
  Lower_CI = round(exp(coef(m4sugar) - 1.96 * robust_se), 3),
  Upper_CI = round(exp(coef(m4sugar) + 1.96 * robust_se), 3),
  P_value = round(pvalue, 4)
) %>%
  mutate(
    PR = paste0(Estimate, " (", Lower_CI, ", ", Upper_CI, ")"),
...
vif(m4sugar) #checking multicollinearity
```

### Soda

```{r}
m3soda <- glm(cariessev_num ~ soda, data = ensab, family = "binomial"(link = "log"))
summary(m3soda)

m4soda <- glm(cariessev_num ~ soda + age + sex + region + ses + income + insurance + location,
             family = poisson(link = "log"), data = ensab)
summary(m4soda)
coeftest(m4soda, vcov = sandwich)# Robust standard errors
exp(m4soda$coefficients) #PR

robust_se <- coeftest(m4soda, vcov = sandwich)[, 2]  # Extract robust standard errors
pvalue <- coeftest(m4soda, vcov = sandwich)[, 4]

soda_ci2 <- data.frame(
  Estimate = round(exp(coef(m4soda)), 3),
  Lower_CI = round(exp(coef(m4soda) - 1.96 * robust_se), 3),
  Upper_CI = round(exp(coef(m4soda) + 1.96 * robust_se), 3),
  P_value = round(pvalue, 4)
) %>%
  mutate(
    PR = paste0(Estimate, " (", Lower_CI, ", ", Upper_CI, ")"),
...
vif(m4soda) #checking multicollinearity
```

### Juice

```{r}
m3juice <- glm(cariessev_num ~ juice, data = ensab, family = "binomial"(link = "log"))
summary(m3juice)

m4juice <- glm(cariessev_num ~ juice + age + sex + region + ses + income + insurance + location,
             family = poisson(link = "log"), data = ensab)
summary(m4juice)
coeftest(m4juice, vcov = sandwich)# Robust standard errors
exp(m4juice$coefficients) #PR

robust_se <- coeftest(m4juice, vcov = sandwich)[, 2]  # Extract robust standard errors
pvalue <- coeftest(m4juice, vcov = sandwich)[, 4]

juice_ci2 <- data.frame(
  Estimate = round(exp(coef(m4juice)), 3),
  Lower_CI = round(exp(coef(m4juice) - 1.96 * robust_se), 3),
  Upper_CI = round(exp(coef(m4juice) + 1.96 * robust_se), 3),
  P_value = round(pvalue, 4)
) %>%
  mutate(
    PR = paste0(Estimate, " (", Lower_CI, ", ", Upper_CI, ")"),
...
vif(m4juice) #checking multicollinearity
```
...
tfoodcariessev <- bind_rows(carbs_ci2, veg_ci2, fruit_ci2, prot_ci2, dairy_ci2, fats_ci2, sugar_ci2, soda_ci2, juice_ci2)

tfoodcariessev

prot_ci2
#export as csv
write.csv(tfoodcariessev, "tfoodcariessev.csv", row.names = FALSE)
```



## Ordinal or multinomial logistic regression for caries severity 

Ordinal has two types: 

* non-proportional odds model
Interpretation of beta 1: log odds of being in a higher category compared to a lower category, for each unit (category frequency of consumption) increase.
It has a separate log OR for every pair of comparison

* Proportional odds model:
INterpretation of beta 1: log odds of being in a higher vs lower category, comparing exposed vs unexposed
Only one log OR

Test proportional odds assumption to choose between POM and non POM. Can be tested with the likelihood ratio test: 
null model POM, extended non POM

```{r}
library(MASS) #for ordinal logistic regression, supports POM
if(!require("VGAM")) install.packages("VGAM")
library(VGAM) #for ordinal logistic regression, supports non POM

summary(ensab$cariessev) # binary
summary(ensab$cariessev3) # ordinal: the one we will use
levels(ensab$cariessev3)
...
pomcarbs1 <- vglm(cariessev3 ~ carbs,
  data = ensab, family = cumulative(parallel = TRUE))
summary(pomcarbs1) #already includes exponentiated coefficients
exp(coef(pomcarbs1))
exp(confint(pomcarbs1))

#fit multivariate POM

pomcarbs2 <- vglm(cariessev3 ~ carbs + age + sex + region + ses + income + insurance + location,
  data = ensab, family = cumulative(parallel = TRUE))
summary(pomcarbs2)
exp(coef(pomcarbs2))
exp(confint(pomcarbs2))

#fit bivariate non POM

npomcarbs1 <- vglm(cariessev3 ~ carbs, data = ensab, family = cumulative(parallel = FALSE))
summary(npomcarbs1)
exp(coef(npomcarbs1))
exp(confint(npomcarbs1))

#fit multivariate non POM

npomcarbs2 <- vglm(cariessev3 ~ carbs + age + sex + region + ses + income + insurance + location, data = ensab, family = cumulative(parallel = FALSE ~ carbs))
summary(npomcarbs2)
exp(coef(npomcarbs2))
exp(confint(npomcarbs2))


#check proportional odds assumption: with likelihood ratio test
VGAM::lrtest(pomcarbs2, npomcarbs2) #Since the p-value > 0.05, fail to reject the null hypothesis that the proportional odds assumption holds
```
...
pomveg1 <- vglm(cariessev3 ~ veg,
  data = ensab, family = cumulative(parallel = TRUE))
summary(pomveg1) #already includes exponentiated coefficients
exp(coef(pomveg1))
exp(confint(pomveg1))

#fit multivariate POM

pomveg2 <- vglm(cariessev3 ~ veg + age + sex + region + ses + income + insurance + location,
  data = ensab, family = cumulative(parallel = TRUE))
summary(pomveg2)
exp(coef(pomveg2))
exp(confint(pomveg2))
...
npomveg1 <- vglm(cariessev3 ~ veg, data = ensab, family = cumulative(parallel = FALSE))
summary(npomveg1)
exp(coef(npomveg1))
exp(confint(npomveg1))

#fit multivariate non POM

npomveg2 <- vglm(cariessev3 ~ veg + age + sex + region + ses + income + insurance + location, data = ensab, family = cumulative(parallel = FALSE ~ veg))
summary(npomveg2)
exp(coef(npomveg2))
exp(confint(npomveg2))


#check proportional odds assumption: with likelihood ratio test
VGAM::lrtest(pomveg2, npomveg2) #Since the p-value > 0.05, fail to reject the null hypothesis that the proportional odds assumption holds for veg
